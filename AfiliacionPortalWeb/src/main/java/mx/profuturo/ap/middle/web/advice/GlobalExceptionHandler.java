package mx.profuturo.ap.middle.web.advice;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ResponseError> handleServiceException(Exception ex, HttpServletRequest request) {
		LOGGER.error("ServiceException Occured:: URL " + request.getRequestURI() + getParameters(request));
	...
	public ResponseEntity<ResponseError> handleSQLException(final Exception ex, final HttpServletRequest request) {
	    ...
	    LOGGER.error("Error detail:: ", ex);
		Exception exceptionSQL = new Exception(
				"Error en la base de datos, favor de contactar al administrador de la aplicaci\u00f3n");
	...
	public ResponseEntity<ResponseError> peticionIncorrecta(final Exception ex, final HttpServletRequest request) {
		...
		ResponseError responseError = createResponseError(ex, HttpStatus.BAD_REQUEST, HttpStatus.BAD_REQUEST.toString(),
				BAD_RQT_EXCEPTION, new ArrayList<String>());
		return new ResponseEntity<>(responseError, HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler({ RestClientException.class })
	public ResponseEntity<ResponseError> restClientException(Exception ex, WebRequest request) {
		ResponseError responseError = createResponseError(ex, HttpStatus.SERVICE_UNAVAILABLE,
				HttpStatus.SERVICE_UNAVAILABLE.toString(), "Error en la cominicacion con el servicio , intente ms tarde.", new ArrayList<String>());
		return new ResponseEntity<>(responseError, HttpStatus.SERVICE_UNAVAILABLE);

	}

	@ExceptionHandler({ ReadValueException.class })
	public ResponseEntity<ResponseError> readValueExceptionException(Exception ex, WebRequest request) {

		ResponseError responseError = createResponseError(ex, HttpStatus.CONFLICT,
				HttpStatus.CONFLICT.toString(), ERROR_BUSINESS_EXCEPTION, new ArrayList<String>());
	...
	protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
		imprimeError(ex);
		final String error = "No se ha encontrado ningún controlador para " + ex.getHttpMethod() + " "
				+ ex.getRequestURL();
	...
	protected ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex,
		...
		HttpMediaTypeNotSupportedException exa = new HttpMediaTypeNotSupportedException(mediaType,
		...
		if (ex != null) {
			if (ex.getMessage() != null) {
				if (ex.getMessage().startsWith("Failed to convert value of type")) {
					...
					mensajeRoot = mensaje
							.replace("Failed to convert value of type", "Fall\u00f3 al convertir el valor de tipo ")
							.replace("to required type", "el tipo requerido es")
							.replace("nested exception is", "la exception es");
				} else {
